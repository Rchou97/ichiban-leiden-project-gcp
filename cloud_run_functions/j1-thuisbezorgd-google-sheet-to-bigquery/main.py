...
def sheet_to_dataframe(key_path: str, gsheets_url: str) -> pd.DataFrame:
    ...
    records_df_unsort = pd.DataFrame.from_dict(records_data)
    records_df_unsort["date"] = pd.to_datetime(records_df_unsort["date"], format='%d-%m-%Y')
    records_df_unsort["date"] = records_df_unsort["date"].dt.strftime("%Y-%m-%d")
    records_df = records_df_unsort.sort_values(by='date', ascending=False)
