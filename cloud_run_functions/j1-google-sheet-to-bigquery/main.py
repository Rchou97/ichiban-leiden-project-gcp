...
def sheet_to_dataframe(key_path: str, gsheets_url: str) -> pd.DataFrame:
    ...
    worksheet = spreadsheet.get_worksheet(1) # get tab 2
    records_data = worksheet.get_all_records()
    records_df_null = pd.DataFrame.from_dict(records_data)
    records_df_unsort = records_df_null.replace("#NUM!", None)
    records_df = records_df_unsort.sort_values('Tijd')
